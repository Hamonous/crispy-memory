...
{	
	...
	{ "Scrap",	   "MyObjectBuilder_Ingot/Scrap" },
...
public Program()
{
    // 1) Gather all text panels that contain either "assembler-status" or "refinery-status".
    ...
    // 2) For each LCD, determine if it is for assemblers or refineries,
...
{
    // 1) Gather all assemblers and sort by name.
    ...
    // 2) Gather all refineries and sort by name.
    ...
    // 3) Update each assembler-status LCD.
    ...
    // 4) Update each refinery-status LCD.
    ...
    try
    {
        ...
        for (int i = 0; i < totalCells; i++)
        {
            ...
            if (i < assemblers.Count)
            {
                ...
                if (!minimalMode)
                {
                    // Move the gauge slightly inward so it doesn't bleed off the left edge.
                    var gaugePos = cellCenter + new Vector2(-85f, -5f);
                    DrawRadialGaugeGradient(frame, gaugePos, 34f, progress, 16);

                    // If blueprint known, draw icon using blueprintIcons_ dictionary.
                ...
                // Move the right text in by 15 pixels.
                float textRight = xStart + cellWidth - 15f;
        ...
        for (int i = 0; i < totalCells; i++)
        {
            ...
            if (i < refineries.Count)
            {
                ...
                if (!minimalMode)
                {
                    ...
                    // Perform inventory queries for input ore and output ingot.
                    var inputItems = new List<MyInventoryItem>();
                    refn.InputInventory.GetItems(inputItems);
                    MyInventoryItem? biggestOre = null;
                    double maxOreAmount = 0;
                    ...
                    MyInventoryItem? biggestIngot = null;
                    double maxIngotAmt = 0;
                    ...
                    // Left side: Ore
                    if (biggestOre.HasValue)
                    {
                        var oreName = biggestOre.Value.Type.SubtypeId;
                        var oreAmount = biggestOre.Value.Amount;
                    ...
                    // Right side: Ingot
                    if (biggestIngot.HasValue)
                    {
                        var ingotName = biggestIngot.Value.Type.SubtypeId;
                        var ingotAmt = biggestIngot.Value.Amount;
                        string itemKey = ingotName + "Ingot";
                        if (itemSprites.ContainsKey(itemKey))
                        {
                            var ingIcon = MySprite.CreateSprite(itemSprites[itemKey], new Vector2(rightX - 10f, sideY - 20f), new Vector2(45, 45));
                            ingIcon.Alignment = TextAlignment.CENTER;
                            frame.Add(ingIcon);
                        }
                        string ingAmtStr = FormatAmount(ingotAmt);
                    ...
                    string resourceName = biggestOre.HasValue ? biggestOre.Value.Type.SubtypeId : "Idle";
